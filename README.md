<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=20&pause=100&color=68FF00&background=000000&center=true&vCenter=true&width=450&height=40&lines=Hello,+I'm+chudengchutx+%F0%9F%91%8B;Code+%7C+TypeScript+%7C+AI+%7C+Life"/>
</p>

<p align="center">
  <img align="center" src="https://github-readme-activity-graph.vercel.app/graph?username=chudengchutx&theme=rogue" />
</p>

<hr style="border: none; border-top: 2px dashed #ccc; margin: 10px 0;">

<table style="width: 100%; border-collapse: collapse;" align="center">
  <tr>
    <td style="width: 50%; padding-right: 20px; vertical-align: top;">
      <h3 style="margin-top: 0; margin-bottom: 5px;">👨‍💻 Code & Projects</h3>
      <p style="margin-top: 0; margin-bottom: 5px;">
        Building with <b>TypeScript</b>. <br/>
        Focus: <a href="https://github.com/chudengchutx/Abu-Cowork"><b>Abu-Cowork</b></a> <br/>
        Exploring: <b>AI Tools, Claude, Open Source</b>
      </p>
    </td>
    <td style="width: 50%; padding-left: 20px; vertical-align: top;">
      <h3 style="margin-top: 0; margin-bottom: 5px;">🌱 More About Me</h3>
      <p style="margin-top: 0; margin-bottom: 5px;">
        🏠 <b>Working from home</b>. <br/>
        ✨ Always learning something new. <br/>
        Let's connect! 👇
      </p>
      <p style="margin-top: 10px;">
        <a href="https://github.com/chudengchutx"><img src="https://img.shields.io/badge/GitHub-24292e?style=for-the-badge&logo=github&logoColor=white"/></a>
      </p>
    </td>
  </tr>
</table>

<hr style="border: none; border-top: 2px dashed #ccc; margin: 10px 0;">

<!-- ===== 可选增强：删掉注释即可启用 =====

统计卡 + 语言卡：
<img src="https://github-readme-stats.vercel.app/api?username=chudengchutx&show_icons=true&theme=rogue" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=chudengchutx&layout=compact&theme=rogue" />

连续提交：
<img src="https://streak-stats.demolab.com?user=chudengchutx&theme=rogue" />

奖杯架：
<img src="https://github-profile-trophy.vercel.app/?username=chudengchutx&theme=rogue" />
-->

## 🗂 我的仓库 · 按最近更新

> 排得越前 = 最近越活跃。🔒 为私有仓库（仅自己可见）。

<!-- OWN_REPOS:START -->
| 仓库 | 说明 | 语言 | 最近更新 |
|---|---|---|---|
| 🔒 **Markdown** | 原生 macOS Markdown 阅读、编辑与分享工具 | Swift | 2026-08-31 |
| 🔒 **oneday** | OneDay macOS 本地优先生活记录工具 | Swift | 2026-08-31 |
| 🔒 **OneDayCapture** | 一日一拍：iPhone 视频日记拍摄、提词、字幕校对与成片导出 | Swift | 2026-08-31 |
| 🔒 **linglipin** | 小区拼单不用愁，「邻里拼」来帮你分忧。一键AI识别商品信息，自动统计费用，告别群内统计乱象，轻松搞定小区拼单。 | JavaScript | 2026-08-31 |
| 🔒 **jianlitong** | 简历通：原生 macOS SwiftUI 简历编辑器 | Swift | 2026-08-31 |
| 🔒 **lucas-site** | — | TypeScript | 2026-08-22 |
| [cola-pages](https://github.com/chudengchutx/cola-pages) | — | — | 2026-08-22 |
| [chudengchutx](https://github.com/chudengchutx/chudengchutx) | — | — | 2026-08-31 |
| [obsidian-advanced-play](https://github.com/chudengchutx/obsidian-advanced-play) | Obsidian 高阶玩法：属性、Bases、知识复利与 Agent 扫库约定 | — | 2026-08-18 |
| 🔒 **personal-website** | — | TypeScript | 2026-08-06 |
| [ai-personal-tools](https://github.com/chudengchutx/ai-personal-tools) | AI-assisted personal tools: membership price parser, job … | JavaScript | 2026-05-05 |
| 🔒 **claude-code-history** | — | — | 2026-04-15 |
| 🔒 **magic** | — | TypeScript | 2026-04-15 |
| 🔒 **movie** | — | — | 2026-04-15 |
| 🔒 **peek** | — | TypeScript | 2026-04-15 |
| 🔒 **EnglishDiary** | 英语学习日记 iOS App — SwiftUI + SQLite + iCloud 同步 | Swift | 2026-04-14 |
| 🔒 **learning-English** | — | Python | 2026-04-14 |
<!-- OWN_REPOS:END -->

## 🍴 Fork 的仓库 · 按最近更新

<!-- FORKS:START -->
| 仓库 | 说明 | 语言 | 最近更新 |
|---|---|---|---|
| [ip-as-logo-skill](https://github.com/chudengchutx/ip-as-logo-skill) | A compact Agent Skill for highly simplified, rounded, sub… | — | 2026-08-19 |
| [grok-app](https://github.com/chudengchutx/grok-app) | Desktop workbench for Grok Build CLI — sessions, projects… | — | 2026-08-18 |
| [md-wechat](https://github.com/chudengchutx/md-wechat) | WeChat Markdown editor & formatter｜公众号 Markdown 排版工具：26 套… | — | 2026-08-12 |
| [LockIME](https://github.com/chudengchutx/LockIME) | A native macOS menu-bar app that keeps your keyboard inpu… | — | 2026-07-21 |
| [sub2api](https://github.com/chudengchutx/sub2api) | Sub2API 一站式开源中转服务，让 Claude、Openai 、Gemini、Grok订阅统一接入，支持拼车… | — | 2026-07-17 |
| [claude-relay-service](https://github.com/chudengchutx/claude-relay-service) | CRS-自建Claude Code镜像，一站式开源中转服务，让 Claude、OpenAI、Gemini、Droi… | — | 2026-07-15 |
| [PixelRAG](https://github.com/chudengchutx/PixelRAG) | The end of web parsing. The beginning of scalable pixel-n… | — | 2026-06-24 |
| [Proma](https://github.com/chudengchutx/Proma) | 把最丝滑的通用 Agent 体验带进你的工作流，为 100x 专业用户而生的未来产品，正在实现 proactive… | — | 2026-06-16 |
| [Arkloop](https://github.com/chudengchutx/Arkloop) | 干净、强大、属于你的 AI Agent 平台  --AI agents, without the clutter. | — | 2026-05-25 |
| [claude-vault](https://github.com/chudengchutx/claude-vault) | 让每次 Claude Code 对话都变成永久知识资产：专属Vibe Coding小白的实践复盘的错题本 | — | 2026-04-16 |
| [magic-resume](https://github.com/chudengchutx/magic-resume) | 基于魔方简历二次开发 | — | 2026-04-15 |
| [Abu-Cowork](https://github.com/chudengchutx/Abu-Cowork) | Claude Cowork 开源替代品，开箱即用，适用于本地使用 | — | 2026-04-12 |
| [ui-ux-pro-max-skill](https://github.com/chudengchutx/ui-ux-pro-max-skill) | An AI SKILL that provide design intelligence for building… | — | 2026-04-03 |
| [Cloud-Drive](https://github.com/chudengchutx/Cloud-Drive) | 一个功能强大的网盘资源自动化管理工具，支持全自动推送每日热更剧集进123网盘，支持全自动监控转存123、115、天… | — | 2025-09-05 |
<!-- FORKS:END -->

<sub>⏰ 仓库榜单由 GitHub Actions 每小时自动刷新，勿手改榜单区</sub>
